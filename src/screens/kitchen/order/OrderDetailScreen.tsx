...
import { OrderDTO, OrderItemDTO, StatusOrder, StatusOrderItem } from '@/models/kitchen/orders/Order'
...
const STATUS_ITEM_LABEL: Record<StatusOrderItem, string> = {
    [StatusOrderItem.CREATED]: 'Pendiente',
    [StatusOrderItem.COMPLETED]: 'Completado',
}

const STATUS_ITEM_CLASSNAME: Record<StatusOrderItem, string> = {
    [StatusOrderItem.CREATED]: 'bg-amber-100 text-amber-700',
    [StatusOrderItem.COMPLETED]: 'bg-emerald-100 text-emerald-700',
}

export function OrderDetailScreen({ order }: OrderDetailScreenProps) {
    ...
    const openPreparedQuantityModal = (item: OrderItemDTO) => {
    ...
    const handleSavePreparedQuantity = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await OrderService.addPreparedQuantity('', {
                orderId: order.id,
    ...
    return (
        <div className="flex flex-col gap-6">
            <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                    <p className="text-sm tracking-wide text-slate-500">Detalle de orden</p>
                    <h1 className="mt-1 text-2xl font-semibold text-slate-900">Orden #{order.id}</h1>
                </div>
            ...
            <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
                    <div>
                        ...
                        <p className="mt-1 text-slate-800">{STATUS_LABEL[order.status]}</p>
                    </div>
                    <div>
                        <p className="text-slate-400">Fecha ordenada</p>
                        <p className="mt-1 text-slate-800">{new Date(order.dateOrdered).toLocaleString('es-ES')}</p>
                    </div>
                    <div>
                        <p className="text-slate-400">Fecha creación</p>
                        <p className="mt-1 text-slate-800">{new Date(order.dateCreatedOn).toLocaleString('es-ES')}</p>
                    </div>
                    <div>
                        <p className="text-slate-400">Items</p>
                        <p className="mt-1 text-slate-800">{orderItems.length}</p>
                    </div>
                </div>
            </section>

            <div className="flex items-center justify-between gap-3">
                <TitlePage title="Items de la Orden" />
            </div>
