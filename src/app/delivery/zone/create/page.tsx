...
export default async function CreateDeliveryZonePage() {
    const token = (await cookies()).get('auth_token')?.value ?? ''

    let drivers: Driver[] = []
    try {
        const response = await DriverService.getAll(token, { page: 1, pageSize: 100 })
    ...
    return (
        <ContainerPage>
            <DeliveryZoneFormScreen drivers={drivers} />
        </ContainerPage>
    )
}
