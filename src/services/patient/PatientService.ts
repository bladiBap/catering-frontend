...
export class PatientService {
    ...
    static async getContactsByPatientId(token: string = '', patientId: string): Promise<ApiResponse<Contact[]>> {
        const response = await axios.get<ApiResponse<Contact[]>>('/patient/patients/contacts/getList', {
            ...(token ? { headers: { Authorization: `Bearer ${token}` } } : {}),
            params: { patientId }
        });
